...
addition和subtractions如你所愿是完全支持的。比如`_mm_add_epi32`表示两个寄存器的32bit lanes相加。
...
进一步，指令集支持对于8、16bit integers的saturated的运算，即不会发生溢出而是固定在最大或最小值。比如，`_mm_adds_epi8( _mm_set1_epi8( 100 ), _mm_set1_epi8( 100 ) )`将返回一个全部8bit lanes为+127的向量，因为虽然和是200,但是一个8bit有符号integer的最大值是+127，点击[这里](http://felix.abecassis.me/2011/10/sse-saturation-arithmetic/) 提供更多关于saturation计算的文章。
...
没有integer divide指令。Intel只在自己编译器的标准库中有integer divide的实现。如果对所有lanes divide一个编译期常量。你可以自己写一个函数，divide单个lane大小的常量，使用https://godbolt.org/编译，扩展到SIMD。例如，对于uint16_t大小的lane divide 11，如下是对应的SIMD指令。
...
对大多数不同大小的integer都有对应的minimum和maximun指令。比如SSE2指令集中的`_mm_max_epu8`计算uint8_t lanes的maximum，SSE4.1指令集中的`_mm_min_epi32`计算32bit interger lanes的minimum。
...
SSSE3指令集中有处理signed integers的absolute value指令。也有horizontal addition和subractions指令，比如`_mm_hsub_epi32`输入[a, b, c, d]和[e, f, g, h]，返回[a-b, c-d, e-f, g-h]。对于signed int16 lanes，horizontal addition 和 subtraction 有对应的saturated版本的指令。
...
我想这是为视频编码设计的。很显然，设计者将AVX2寄存器视为8*4大小的8bit像素区域，然后计算compression errors。这个指令计算每行error的和。我已经多次将其用于视频编解码以外的事情。将`_mm_sad_epu8`的第二个参数设为0，然后使用`_mm_add_epi64`累加，这是最快的方式计算所有字节和。
...
comparisons只有all-lane版本的指令。结果与float comparision相似，将结果lane的所有bit设为全0或者全1。比如，`_mm_cmpgt_epi8`根据对应signed 8bit数值的比较的结果，将对应的8-bit lane设为0或者0xFF。
...
将所有lanes的signed integer的所有bit设为1相当于-1。在comparison后接一个subtract，可以统计符合比较条件的数量。
...
注意integer的溢出。当累加32bit大小的integer时一般并不需要考虑（使用SSE2指令集时，当累加大小为4G的向量即64GB大小后才会发生溢出），但是当使用8或者16bit的integer时就很危险了。一种处理办法是使用嵌套循环，在内层循环中计算部分数值，保证累加器不发生溢出，在外层循环中提升到更宽的integer lanes。
...
对于integer lanes比较a<=b，一个好的方式是`min( a, b ) == a`（由两个很快的指令组成）。move mask指令只支持8bit lanes。如果你想将32bit integers的比较结果放在通用寄存器中，每个bit表示一个lane的结果。可以将`__m128i` cast `__m128`然后使用`_mm_movemask_ps`。同样对于64bit lanes，cast `__m128d`然后使用`_mm_movemask_pd`。
...
SSE2指令集中整体shift寄存器的指令有`_mm_srli_si128`和`_mm_slli_si128`。在AVX2中相近的指令是`_mm256_slli_si256`，可以分别shift两个16byte lanes，shift空出的bit被填充为0。如果你想shift整个32byte的寄存器，可以看[stackoverflow](https://stackoverflow.com/questions/66179765)的答案。
...
SSSE3指令集的`_mm_alignr_epi8`指令，传入两个16字节寄存器作为参数，concatenate为一个32bit的临时变量然后整体shift，最后保留低1字节。
...
不同于floats，`__m128i`和`__m256i`有很多种lanes。不同的指令处理包括有符号和无符号在内的8-、16-、32-和64-bit的lanes。以下是一些pack或unpack的指令。
...
在AVX2指令集之前，将无符号integer convert到更大的宽度是最好的方式是与0做unpack。AVX2指令集中引入了更合适的指令，比如`_mm256_cvtepu8_epi32`将原数据的8个最低的字节convert为8个32bit的数值，这些指令甚至可以直接从内存load数据。
...
同样在AVX2指令集中对应`_mm256_shuffle_epi8`，将整个256bit的分为两半，分别使用相同的shuffle规则，第二个参数用来控制shuffle。