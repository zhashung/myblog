...
最近在学习SIMD的指令，但是并没有找到非常好的中文资料。在网络上偶然看到此文章《SIMD for c++ developers》，对Intel SIMD指令进行了整体介绍，工作之便，以作翻译分享。本人对SIMD尚不熟悉，因此翻译进行的比较缓慢，问题较多，还请多多指正。
## Introduction
...
...
这项技术的主要难度是陡峭的学习曲线。我花了数月时间入门，大约过了一年才能流畅地使用它。我有着数十年的c++编程经验，多年的GPU编程经验。然而这些都没有用，CPU的SIMD与这些完全不同。
...
首先，我们需要时间去理解这个概念。现在所有的CPU实际上都支持向量操作，每次你编写的`float a = b + c`，你实际浪费了75%-85%的CPU时间。因为你使用一些指令在相同的时间同时计算4-8组数字的加法。
...
然后，我们需要花费更多的时间熟悉指令。在刚开始学习这项技术时，我花费数小时编写的代码发现与标量版本相差无几，后来在使用了一些特殊的指令并仅用了少量代码后得到了数量级的性能提升，这些事情屡见不鲜。
...
另外，文档并不友好。我希望去改进它。但是，目前官方文档仍然只对熟悉指令的使用者有用。多数开发者并不知道SIMD的用途。我写这篇文章也是希望可以改善这种情况。
...
多数情况，你应该使用寄存器处理数据。技术上讲，许多指令可以直接操作内存，但这并不是一个好主意。使用SIMD代码工作的理想模式是：加载数据到寄存器，尽可能在寄存器中处理这些数据，然后存回内存。最理想的情况是：当没有处理结果，或者结果是一个很小的数比如bool或者单个integer，某些指令可以把数据从SIMD寄存器复制到通用寄存器。
...
在C++中，SIMD寄存器包括6种基本类型的变量。如右表格所示。在汇编代码中，相同大小的寄存器之间没有任何区别，类型仅是为了c++的类型检查。
...
编译器自动分配变量到寄存器。但是只有16个SIMD寄存器，对于编译32位代码则更是只有8个。如果你定义了过多局部变量，并且代码中变量之间依赖性较高，编译器将把部分变量从寄存器迁移到RAM上的栈区。对于一些边界情况，这将损失性能。测试软件，检查编译器如何处理那些关乎性能的代码部分。
...
对于这些向量类型，需要注意：编译器将这些向量定义为一个structure或者union，内部则是数组。技术上，你可以通过访问数组的方式获取向量中单独的lane，代码是可以正常编译并工作的，虽然性能不佳。因为通常会造成数据往返于内存和寄存器之间。因此不要用这种方式使用SIMD向量，使用shuffle、insert或者exrtact的intrinsics更为合适。
...
但是在C++中，两者是不同的，因此存在intrinsics用来转换两种类型。如下表，行表示原始数据类型，列表示目标数据类型。
...
所有这些intrinsics不会编译成任何指令，因此是不耗费任何性能的。这些intrinsics不改变寄存器中任何bit，因此，32bit float `1.0f` 将cast 32bit integer `0x3f800000`。当cast 16字节到32字节时，高字节部分是未定义的（undefined）。
...
有一些指令可以执行类型conversion。目前只支持有符号的32bit integer lanes。`_mm_cvtepi32_ps` convert 4个32bit interger lanes至32bit float。`_mm_cvtepi32_pd`convert 4个double至interger，高4个lanes置为0。当从float convert至integer时，round采用MXCSR或status register中指定的模式。通过`_MM_SET_ROUNDING_MODE`宏可以修改round模式。That value is a part of thread state, so the OS persists the value across context switches。（这一特性可能导致在线程池环境下很难发现数值错误，比如OpenMP。因此，相对于更换MXCSR我更喜欢使用SSE4.1中的round intrinsics）
...
- `__m128`和`__m128d`支持一些额外的load方式。`_mm_load_ss`和`_mm_load_sd`支持single-lane load，即只load第一个lane，并设置其他lanes为`0.0`。对于aligned的数据，一些intrinsics支持倒序load。
...
- 在AVX指令集中`__m128`、`__m256`、`__m256d` 有broadcast load指令，可以从内存中load single float或者double到目标的所有lanes中。有些指令可以从内存中读取16字节到`__m256`或`__m256d`，然后复制到（向量）寄存器的另一半。

- AVX 1 introduced masked load instructions, they load some lanes and zero out others.

- AVX1中引入了masked load 指令，可以只load一部分lanes，其他的置为0。
...
- AVX2中进入了gather load指令，如`_mm_i32gather_ps`，可以load 原始指针、带偏移的interger、带缩放的interger。方便的同时，比较慢。
...
同样，在AVX1中，针对aligned、unaligned、single-lane、masked有多种方式store。
...
现代计算机，RAM 每次load至少load8字节。dual-channel内存，每次load 16字节。因此读取aligned的16或者32字节块是很快的。如果你查看现代memcpy或memmove函数在当下的源代码，你可以看到使用SSE2手动向量化复制字节。同样当你transpose时，load连续的16或32字节块到寄存器中，然后使用shuffle指令通常更快。

1. Many instructions support memory source for one of the operands. When compiling for SSE-only targets, this requires aligned loads. When compiling for AVX targets, this works for both aligned and unaligned loads. Code like `_mm_add_ps( v, _mm_loadu_ps( ptr ) )` only merges the load when compiling for AVX, while `_mm_add_ps( v, _mm_load_ps( ptr ) )` merges for all targets because the load is aligned (but might crash in runtime if the passed address is not actually amultiple of 16 bytes).

很多指令支持memory source for one of the operands。当只使用SSE编译目标时，要求aligned load。当编译AVX时，aligned或unaligned load均可。`_mm_add_ps( v, _mm_loadu_ps( ptr ) )`只当使用AVX编译时，load才能被merge。`_mm_add_ps( v, _mm_load_ps( ptr ) )`load被merge是因为是数据是aligned的（但是如果运行时传入的并不是16字节aligned的，则会发生崩溃）。
...
多数single-lane的loads或store只支持操作寄存器的最低lane。如果你想要实现reduction比如sum或average，将结果存放在第一个lane。除了一些AVX指令 `_mm256_insertf128_ps`, `_mm256_insertf128_pd`, `_mm256_inserti128_si256`，这些指令load 16字节的向量到寄存器的高一半部分，而且相对常规的16字节load相比几乎没有性能损失。
...
所有类型向量都有对应的intrinsics，比如`_mm_setzero_ps`和`_mm256_setzero_si256`，将寄存器全部初始化为0。代码编写的形式如xorps xmm0, xmm0, xmm0。初始化为0是很快的因为不依赖于XOR指令，CPU只需要rename一个新的寄存器。（硬件超过16个，只不过有16个名称。硬件实际上有上千个，流水线很深，包括很多动态指令，这些指令需要多个版本的相同逻辑的寄存器）
...
CPU不能将寄存器初始化除了0以外的常量，但是编译器假装可以。`_mm[256]_set_something`可以将lanes初始化不同的值。`_mm[256]_set_something`可以初始化全部lanes一个值。如果变量是编译期常量，该变量将编译为二进制文件的read-only data区域。编译器emit完整的16/32bytes数值，因此保证数据是aligned。如果变量在编译期是未知的，编译器会做一些合理性动作，比如寄存器除了1个lane其他全是0，编译器会使用emit `_mm_insert_something` 指令。如果值从变量中拷贝而来，编译器会在load之后使用shuffles、scalar stores等。
...
对于所有`_mm[256]_set_something`，Intel有一些问题。当往寄存器中存入`[ 1, 2, 3, 4 ]`时，你需要使用`_mm_set_epi32( 4, 3, 2, 1 )`或者`_mm_setr_epi32( 1, 2, 3, 4 )`。
...
floats和interger都有几乎完整的bitwise指令，包括 AND，OR，XOR和ANDNOT。如果你需要bitwise NOT，最快的方式是与1做XOR，比如对于16byte：
...
当你需要全1的寄存器，最好的方式如上所示，setzero然后0与0比较。