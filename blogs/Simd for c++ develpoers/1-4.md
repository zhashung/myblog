...
多数情况，你应该使用寄存器处理数据。技术上讲，许多指令可以直接操作内存，但这并不是一个好主意。编写SIMD代码的理想模式是：加载数据到寄存器，尽可能在寄存器中处理这些数据，然后存回内存。最理想的情况是：不需要返回结果时，或者返回结果是一个很小的数比如bool或者integer时，再使用某些指令把数据从SIMD寄存器复制到通用寄存器。
...
但是在C++中，两者是不同的，因此存在指令用来转换两种类型。如下表，第一行表示原始数据类型，第一列表示目标数据类型。
...
有一些指令可以执行类型的 conversion。目前只支持有符号的32bit integer lanes。`_mm_cvtepi32_ps` convert 4个32bit interger lanes至32bit float。`_mm_cvtepi32_pd` convert 前两个integers至64bit float，`_mm256_cvtpd_epi32` convert 4个double至interger，剩余的高4个lanes置为0。当从float convert至integer时，round采用MXCSR或status register中指定的模式。通过`_MM_SET_ROUNDING_MODE`宏可以修改round模式。这个值属于当前线程状态，因此当操作系统执行上下文切换时将暂存这个值。（这一特性可能导致在线程池环境下很难发现数值错误，比如OpenMP。因此，相对于更换MXCSR我更喜欢使用SSE4.1指令集中的round指令）
...
支持所有类型的常规load，比如`_mm_load_si128`和`_mm256_load_ps`。但是这些指令要求原始数据的地址是16bytes或32bytes aligned的，否则可能会因为编译器设置造成崩溃。
...
所有数据类型支持unaligned load，比如`_mm_loadu_si128`和`_mm256_loadu_ps`。可以正常加载unaligned指针，但是使用aligned loads比unaligned load更快。
...
AVX2指令集中加入了gather load指令，如`_mm_i32gather_ps`，可以从起始指针处gatherinteger，并同时对指针位置进行偏移和缩放。方便的同时，比较慢。
...
最后，在很多情况，你在load时不需要做任何特殊的操作。如果你的数据是aligned，你可以直接写做`__m128i value = *pointer`；编译器会将其编译为对应的load指令。
...
现代计算机，RAM每次load至少load 8字节。对于双通道内存，每次load 16字节。因此读取aligned的16或者32字节块是很快的。如果你查看memcpy或memmove函数最新的库程序，你可以看到使用SSE2指令使用向量化指令复制字节。同样当你transpose时，load连续的16或32字节块到寄存器中，然后使用shuffle指令通常更快。
...
有一些指令如`_mm[256]_stream_something`可以绕过CPU缓存 load store数据。在视频处理时会有用。当你写入的内容很后面才可能被使用, after you’ve processed the complete frame, and probably by another CPU core (L1 and L2 caches are per-core, only L3 is shared).
...
CPU不能将寄存器初始化除了0以外的常量，但是编译器假装CPU可以。`_mm[256]_set_something`可以将lanes初始化不同的值，`_mm[256]_set1_something`可以初始化全部lanes为一个值。如果变量是编译期常量，该变量将编译为二进制文件的只读区域。编译器编译出完整的16/32bytes数值，因此保证数据是aligned。如果变量在编译期是未知的，编译器会做一些合理性动作，比如寄存器只有一个lane不是0，编译器会使用使用`_mm_insert_something` 指令。如果值从变量中拷贝而来，编译器会在load之后使用shuffles、scalar stores等。
